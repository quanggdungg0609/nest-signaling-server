...
export class WebsocketGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleRequestListUsers(client:WebSocket, data:any){
    this.authCameraAndDoAction(client,()=>{
      let listUsers = Array.from(this.users.values()).map(user => ({ uuid: user.uuid }))
  ...
  handleRequestListCameras(client:WebSocket, data:any){
    this.authUserAndDoAction(client,()=>{
      let listCameras = Array.from(this.cameras.values()).map(camera=>({
        uuid: camera.uuid,
        name: camera.name,
  ...
  handleOfferSD(client: WebSocket, data:{uuid: string, to:string, type: string, sdp:string,}){
  ...
  handleAnswerSD(client: WebSocket, data:{uuid:string, to: string, type:string, sdp: string}){
  ...
  private authCameraAndDoAction(userSocket: WebSocket, onAuthorized: () => void):void{
  ...
  private authUserAndDoAction(userSocket:WebSocket, onAuthorized:()=>void):void{
