...
import { GetThumbnailVideoDto, GetThumbnailImageDto, GetVideoNamesDto } from '../DTOs';
...
export class FilesController {
    ...
    async getThumbnail(@Param('camaraUuid') dto: GetThumbnailImageDto, @Res() res:Response){
        try{
            const imgPath = await this.filesService.getThumbnail(dto.cameraUuid)
    ...
    async getThumbnailVideo(@Param() getThumbnailDto: GetThumbnailVideoDto ,@Res() res: Response){
        ...
        try{
            const imgPath = await this.filesService.getThumbnailVideo(cameraUuid, imageName)
            res.sendFile(imgPath)
            return
        }catch(error){
            if(error instanceof NotFoundException){
    ...
    @ApiOperation({ summary: 'Get video stream with the name of video and the uuid of camera '})
    @ApiResponse({
        status: 200,
    ...
    @ApiParam({ name: 'videoName', description: 'Name of video', type: 'string', example: "152105_01052024" })
    @ApiParam({ name: 'cameraUuid', description: 'UUID of the camera', type: 'string', example: "a9f0cef0-68cf-43dc-a3a8-5242212cb330" },)
    async getStreamVideo(
        ...
        try{
            const videoPath = this.filesService.getVideoPath(dto.cameraUuid, dto.videoName)
